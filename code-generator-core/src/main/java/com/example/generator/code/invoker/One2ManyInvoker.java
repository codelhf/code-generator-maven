...
public class One2ManyInvoker extends BaseInvoker {
    ...
    protected void initTasks() {
        ...
        taskQueue.add(new ServiceImplTask(className, false));
        taskQueue.add(new DaoTask(className, false));
        taskQueue.add(new MapperTask(tableName, className, parentTableName, parentClassName, foreignKey, tableInfo, parentTableInfo, false));
        taskQueue.add(new EntityTask(className, parentClassName, foreignKey, tableInfo));
        taskQueue.add(new EntityTask(parentClassName, parentTableInfo));
        taskQueue.add(new EntityDtoTask(className, parentClassName, foreignKey, tableInfo));
