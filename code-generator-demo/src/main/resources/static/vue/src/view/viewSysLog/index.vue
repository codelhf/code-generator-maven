<template>
  <div class="page-container">
      ...
      <el-row>
        <el-col :span="24" style="text-align: center">
          <el-form-item>
            ...
            <el-button type="primary" icon="el-icon-plus" @click="handleCreate">新增ViewSysLog</el-button>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
    ...
    <!-- dialog表单 -->
    <el-dialog
      :title="viewSysLog.id ? '修改ViewSysLog' : '新增ViewSysLog'"
      :visible.sync="dialogFormVisible"
      @close="handleFormClose('viewSysLogForm')"
    >
      <el-form ref="viewSysLogForm" :model="viewSysLog" :rules="viewSysLogRules" label-width="130px" label-suffix=":">
        <el-form-item label="记录id" prop="id">
          <el-input v-model="viewSysLog.id" placeholder="请输入记录id" />
        </el-form-item>
        <el-form-item label="权限更新的类型，1：部门，2：用户，3：权限模块，4：权限，5：角色，6角色用户关系，7角色权限关系" prop="type">
          <el-input v-model="viewSysLog.type" placeholder="请输入权限更新的类型，1：部门，2：用户，3：权限模块，4：权限，5：角色，6角色用户关系，7角色权限关系" />
        </el-form-item>
        <el-form-item label="基于type制定后的对象id，比如用户，权限，角色等表的主键" prop="targetId">
          <el-input v-model="viewSysLog.targetId" placeholder="请输入基于type制定后的对象id，比如用户，权限，角色等表的主键" />
        </el-form-item>
        <el-form-item label="旧值" prop="oldValue">
          <el-input v-model="viewSysLog.oldValue" placeholder="请输入旧值" />
        </el-form-item>
        <el-form-item label="新值" prop="newValue">
          <el-input v-model="viewSysLog.newValue" placeholder="请输入新值" />
        </el-form-item>
        <el-form-item label="操作者" prop="operator">
          <el-input v-model="viewSysLog.operator" placeholder="请输入操作者" />
        </el-form-item>
        <el-form-item label="最后一次操作时间" prop="operateTime">
          <el-input v-model="viewSysLog.operateTime" placeholder="请输入最后一次操作时间" />
        </el-form-item>
        <el-form-item label="最后一次更新者的IP" prop="operateIp">
          <el-input v-model="viewSysLog.operateIp" placeholder="请输入最后一次更新者的IP" />
        </el-form-item>
        <el-form-item label="当前是否复原过，0：没有，1：复原过" prop="status">
          <el-input v-model="viewSysLog.status" placeholder="请输入当前是否复原过，0：没有，1：复原过" />
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="handleFormClose('viewSysLogForm')">返回</el-button>
        <el-button type="primary" @click="handleFormSubmit('viewSysLogForm')">确定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
<script>
import { fetchViewSysLogList, fetchViewSysLog, createViewSysLog, updateViewSysLog, deleteViewSysLog } from '@/api/viewSysLog/index';
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchViewSysLogList(this.listQuery)
        .then(response => {
    ...
    handleEdit(index, row) {
      fetchViewSysLog(row.id).then(res => {
        this.dialogFormVisible = true;
        this.viewSysLog = res;
      });
    },
    ...
    handleFormSubmit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (!this.viewSysLog.id) {
            createViewSysLog(this.viewSysLog).then(res => {
              this.dialogFormVisible = false;
              this.getList();
            });
          } else {
            updateViewSysLog(this.viewSysLog).then(res => {
              this.dialogFormVisible = false;
              this.getList();
            });
          }
